...
public class QueryRegenerateEvent extends ServerEvent {
    ...
    public QueryRegenerateEvent(Server server, int timeout) {
        ...
        this.serverName = server.getSettings().general().motd();
        this.listPlugins = server.getSettings().network().queryPlugins();
        ...
        this.gameType = server.getSettings().player().defaultGamemode() == 1 ? "CMP" : "SMP";
        this.version = server.getVersion();
        this.map = server.getDefaultLevel() == null ? "unknown" : server.getDefaultLevel().getName();
        this.numPlayers = this.players.length;
        this.maxPlayers = server.getMaxPlayers();
        this.whitelist = server.getSettings().player().whitelist() ? "on" : "off";
